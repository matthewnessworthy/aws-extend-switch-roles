...
**Publish mechanics.** Chrome publishes via a self-contained `curl` step against the Chrome Web Store API **v1.1** (no third-party publish action). ⚠️ v1.1 is supported only until **2026-10-15**; before then, migrate the Chrome step to the v2 API (`chromewebstore.googleapis.com`), which additionally requires a publisher id. Edge publishes via the maintained `wdzeng/edge-addon` action (Edge Add-ons API v1.1). The previously-used `PlasmoHQ/bpp` action was **removed** — it is dormant (last release Feb 2025) and stuck on the deprecated Node 20 action runtime. Firefox is unchanged (`web-ext`, see below).
