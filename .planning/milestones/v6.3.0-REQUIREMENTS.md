# Requirements Archive: v6.3.0 UI Modernization
...
- [x] **FND-01**: A shared CSS design-token system (custom properties for color, type, spacing, radius, elevation) is consumed by every surface
- [x] **FND-02**: Tokens are hand-derived from the AWS Cloudscape Visual Refresh palette (light + dark), with no `@cloudscape-design/*` package (including the CSS-only `design-tokens`) and no JS/CSS framework
- [x] **FND-03**: CSS ships as static files copied into both Chrome and Firefox builds (`bin/build.sh` and `bin/build_test.sh` updated); not routed through Rollup
- [x] **FND-04**: Typography uses a system font stack — no bundled webfont
...
- [x] **THM-03**: No flash of the wrong theme on popup or options load (FOUC eliminated via a pre-paint, non-module `theme-init.js`)
- [x] **THM-04**: Theme choice persists and follows the user across devices; already-open pages update live when it changes
- [x] **THM-05**: Native controls and scrollbars follow the active theme on Chrome, Firefox, and Edge (`color-scheme` set per theme)

### Popup

- [x] **POP-01**: The popup role list is restyled to the Cloudscape-native treatment within the 600×600 cap
- [x] **POP-02**: Long role names / account IDs truncate gracefully; the filter stays usable at minimum width
- [x] **POP-03**: The popup shows clear empty states, distinguishing "not on an AWS console page" from "on AWS page but no matching roles," with guidance
- [x] **POP-04**: The popup shows a loading state while roles resolve
- [x] **POP-05**: Existing keyboard navigation (`Ctrl+Shift+,`, arrow-key list nav, type-to-filter) is preserved and clearly focus-visible
- [x] **POP-06**: The per-profile color swatch renders legibly in both themes (render-only; the stored value and console-header behavior are unchanged)
...
| POP-02 | Phase 2 | Complete |
| POP-03 | Phase 2 | Complete |
| POP-04 | Phase 2 | Complete |
| POP-05 | Phase 2 | Complete |
| POP-06 | Phase 2 | Complete |
